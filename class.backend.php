...
class formBackend {
    ...
    public function checkFormEdit() {
                ...
                case dbKITform::field_name:
                    ...
                    if (count($result) > 0) {
                        if (($form_id > 0) && ($result[0][dbKITform::field_id] !== $form_id)) {
                            ...
                            $message .= $this->lang->translate('<p>The form name can not changed to <b>{{ name }}</b>, this name is already in use by the form with the <b>ID {{ id }}</b>.</p>', 
                                    array('name' => $name, 'id' => sprintf('%03d', $result[0][dbKITform::field_id])));
                            unset($_REQUEST[$field]);
                            $checked = false;
                            break;
                        } elseif ($form_id < 1) {
                            // Der Bezeichner wird bereits verwendet
                            $message .= $this->lang->translate('<p>The name <b>{{ name }}</b> is already in use by the form with the <b>ID {{ id }}</b>, please use another name!</p>', 
                                    array('name' => $name, 'id' => sprintf('%03d', $result[0][dbKITform::field_id])));
                            unset($_REQUEST[$field]);
                            $checked = false;
                            break;
        ...
        foreach ($fields as $key => $value) {
            if ($value < 100) {
                ...
                if (! isset($_REQUEST[$field_name])) {
                    $message .= $this->lang->translate('<p>The datafield <b>{{ field }}</b> was removed.</p>', 
        ...
        if ($checked) {
            ...
            if ($form_id > 0) {
                ...
                $message .= $this->lang->translate('<p>The form with the <b>ID {{ id }}</b> was updated.</p>',
                        array('id' => $form_id));
            } else {
                // Datensatz einfuegen
                ...
                $message .= $this->lang->translate('<p>The form with the <b>ID {{ id }}</b> was successfully created.</p>',
                        array('id' => $form_id));
            }
        ...
        if (isset($_REQUEST[self::request_add_kit_field]) && (array_key_exists($_REQUEST[self::request_add_kit_field], $kit_fields))) {
            ...
            if ($form_id > 0) {
                ...
                $message .= $this->lang->translate('<p>The KIT datafield <b>{{ field }}</b> was added to the form.</p>',
                        array('field' => $kitContactInterface->field_array[$new_field]));
            } else {
                // Formular ist noch nicht aktiv
                $message .= $this->lang->translate('<p>Please save the new form before you insert the datafield <b>{{ field }}</b>!</p>',
                        array('field' => $kitContactInterface->field_array[$new_field]));
            }
        }
        ...
        if (isset($_REQUEST[self::request_add_free_field]) && ($_REQUEST[self::request_add_free_field] != - 1)) {
            if (isset($_REQUEST[self::request_free_field_title]) && 
                    ($_REQUEST[self::request_free_field_title] !== $this->lang->translate('Enter title ...')) && 
                    ! empty($_REQUEST[self::request_free_field_title])) {
                if ($form_id > 0) {
                    ...
                    $message .= $this->lang->translate('<p>The general datafield <b>{{ field }}</b> was added to the form.</p>', 
                            array('field' => $_REQUEST[self::request_free_field_title]));
                } else {
                    // Formular ist noch nicht aktiv
                    $message .= $this->lang->translate('<p>Please save the new form before you insert the datafield <b>{{ field }}</b>!</p>',
                            array('field' => $_REQUEST[self::request_free_field_title]));
        ...
        // Sortierung pruefen
        $where = array(dbKITformTableSort::field_value => $form_id, 
    ...
    public function dlgFormEdit() {
        ...
        foreach ($form_data as $field => $value) {
            switch ($field) :
                ...
                case dbKITform::field_provider_id:
                    ...
                    $form[$field]['items'] = array();
                    $form[$field]['items'][] = array('value' => - 1, 
                ...
                case dbKITform::field_email_html:
                    ...
                    foreach ($dbKITform->html_array as $value => $text) {
                        $form[$field]['items'][] = array('value' => $value, 
                        'text' => $text);
                    }
                    $form[$field]['label'] = $this->lang->translate('label_' . $field);
                    $form[$field]['name'] = $field;
        ...
        foreach ($field_array as $field => $text) {
            ...
            $form['kit_field']['value'][] = array('value' => $field, 
            'text' => $text);
        }
        ...
        $form['free_field']['value'][] = array('value' => - 1, 
        'text' => $this->lang->translate('- select datafield -'));
        $field_array = $dbKITformFields->type_array;
        asort($field_array);
        foreach ($field_array as $field => $text) {
            $form['free_field']['value'][] = array('value' => $field, 
            'text' => $text);
        }
        ...
        if ($form_id > 0) {
            ...
            if (count($sorter) < 1) {
                $data = array(dbKITformTableSort::field_table => $sorter_table, 
    ...
    public function dlgFormList() {
        ...
        foreach ($forms as $form) {
            $list[] = array('id' => $form[dbKITform::field_id], 
        ...
        $SQL = sprintf("SELECT %s FROM %s WHERE %s!='%s' AND %s<'1'",
                dbKITform::field_id,
                $dbKITform->getTableName(),
                dbKITform::field_status,
                dbKITform::status_deleted,
                dbKITform::field_provider_id);
        $providers = array();
    ...
    public function dlgProtocolList() {
        ...
        $SQL = sprintf("SELECT * FROM %s ORDER BY %s DESC LIMIT 100", $dbKITformData->getTableName(), dbKITformData::field_date);
        $items = array();
        ...
        foreach ($items as $item) {
            ...
            if (count($form) < 1) {
                $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, sprintf(kit_error_invalid_id, $item[dbKITformData::field_form_id])));
                return false;
            }
            $form = $form[0];
    ...
    public function dlgProtocolItem() {
        ...
        if (count($protocol) < 1) {
            $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, sprintf(kit_error_invalid_id, $protocol_id)));
            return false;
        }
        ...
        if (count($form) < 1) {
            $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, sprintf(kit_error_invalid_id, $protocol[dbKITformData::field_form_id])));
            return false;
        }
        $form = $form[0];
        ...
        foreach ($form_fields as $fid) {
            ...
            endswitch
            ;
            $items[] = array('label' => $field[dbKITformFields::field_title], 
            'value' => $value);
        }
    ...
    public function exportForm() {
        ...
        $export = array('version' => $this->getVersion(), 'form' => $form, 
        'fields' => $fields);
        ...
        $this->setMessage($this->lang->translate('<p>The form was successfully exported as <b><a href="{{ url }}">{{ name }}</a></b>.</p>',
                array('url' => $file_url, 'name' => basename($file_url))));
        return true;
    } // exportFormDlg()
