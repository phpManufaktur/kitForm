    ...
    public function importFormFile($form_file, $form_rename = '', &$form_id = -1, &$message = '', $ignore_existing = false) {
        ...
        if (false === ($import = file_get_contents($form_file))) {
            $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, 
                    $lang->translate('<p>Error reading the file <b>{{ file }}</b>.</p>', array('file' => basename($form_file)))));
            return false;
        }
        $form_data = array();
        ...
        if (empty($form_data)) {
            $message = $lang->translate('<p>The file <b>{{ file }}</b> does not contain valid form datas.</p>',
                    array('file' => basename($form_file)));
            return false;
        }
        
        $version = (float) $form_data['version'];
        if (! is_float($version)) {
            $message = $lang->translate('<p>The file <b>{{ file }}</b> does not contain valid version informations!</p>',
                    array('file' => basename($form_file)));
            return false;
        }
        ...
        if (count($form_check) > 0) {
            ...
            if (! $ignore_existing) $message = $lang->translate('<p>The form name <b>{{ name }}</b> is already in use, the import of <b>{{ file }}</b> was aborted.</p>',
                    array('name' => $form_name, 'file' => basename($form_file)));
            return false;
        }
