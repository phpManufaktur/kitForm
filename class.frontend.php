...
class formFrontend {
	...
	private $params = array(self::PARAM_PRESET => 1, self::PARAM_FORM => '', self::PARAM_RETURN => false, self::PARAM_CSS => true, self::PARAM_AUTO_LOGIN_LEPTON => false, self::PARAM_LANGUAGE => KIT_FORM_LANGUAGE, self::PARAM_FALLBACK_LANGUAGE => 'DE', self::PARAM_FALLBACK_PRESET => 1, self::PARAM_DEBUG => false);
	...
	protected $general_excluded_extensions = array('php', 'php3', 'php4', 'php5', 'php6', 'phps', 'js', 'htm', 'html', 'shtml');
	...
	public function setParams($params = array()) {
		...
		if (!file_exists($this->template_path . $this->params[self::PARAM_PRESET])) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('The preset directory <b>{{ directory }}</b> does not exists, can\'t load any template!', array('directory' => '/modules/kit_form/htt/' . $this->params[self::PARAM_PRESET] . '/'))));
			return false;
		}
		return true;
	} // setParams()
	...
	protected function getTemplate($template, $template_data) {
			...
			if (!file_exists($template_path)) {
				...
				if (!file_exists($template_path)) {
					...
					if (!file_exists($template_path)) {
						...
						// up!
						$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('Error: The template {{ template }} does not exists in any of the possible paths!', array('template', $template))));
						return false;
					}
				}
			}
		...
		try {
			...
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('Error executing template <b>{{ template }}</b>:<br />{{ error }}', array('template' => $template, 'error' => $e->getMessage()))));
			return false;
		}
		return $result;
	} // getTemplate()
	...
	protected function showForm() {
		...
		// Formulardaten
		$form_data = array('name' => 'kit_form', 'anchor' => self::FORM_ANCHOR, 'action' => array('link' => $this->page_link, 'name' => self::request_action, 'value' => self::action_check_form), 'id' => array('name' => dbKITform::field_id, 'value' => $fdata[dbKitform::field_id]), 'response' => ($this->isMessage()) ? $this->getMessage() : NULL, 'btn' => array('ok' => $this->lang->translate('OK'), 'abort' => $this->lang->translate('Abort')), 'title' => $fdata[dbKITform::field_title], 'captcha' => array('active' => ($fdata[dbKITform::field_captcha] == dbKITform::captcha_on) ? 1 : 0, 'code' => $call_captcha), 'kit_action' => array('name' => dbKITform::field_action, 'value' => $fdata[dbKITform::field_action]), 'links' => $links);
		...
		foreach ($fields_array as $field_id) {
			if ($field_id < 100) {
				...
				if (false === ($field_name = array_search($field_id, $kitContactInterface->index_array))) {
					...
					$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('The field with the <b>ID {{ id }}</b> is no KIT datafield!', array('id' => sprintf('%03d', $field_id)))));
					return false;
				}
				switch ($field_name) :
					...
					case kitContactInterface::kit_title_academic :
						...
						$form_fields[$field_name] = array('id' => $field_id, 'type' => $field_name, 'name' => $field_name, 'value' => '', 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'label' => $kitContactInterface->field_array[$field_name], 'hint' => $this->lang->translate('hint_' . $field_name), 'titles' => $title_array);
						break;
					case kitContactInterface::kit_address_type :
						...
						$form_fields[$field_name] = array('id' => $field_id, 'type' => $field_name, 'name' => $field_name, 'value' => 1, 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'label' => $kitContactInterface->field_array[$field_name], 'hint' => $this->lang->translate('hint_' . $field_name), 'address_types' => $address_type_array);
						break;
					...
					case kitContactInterface::kit_birthday :
						$form_fields[$field_name] = array('id' => $field_id, 'type' => $field_name, 'name' => $field_name, 'value' => (isset($_REQUEST[$field_name])) ? $_REQUEST[$field_name] : '', 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'label' => $kitContactInterface->field_array[$field_name], 'hint' => $this->lang->translate('hint_' . $field_name));
						break;
					...
					case kitContactInterface::kit_free_note_2 :
						...
						$form_fields[$field_name] = array('id' => $field_id, 'type' => $field_name, 'name' => $field_name, 'value' => (isset($_REQUEST[$field_name])) ? $_REQUEST[$field_name] : '', 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'label' => $label, 'hint' => $this->lang->translate('hint_' . $field_name));
						break;
					case kitContactInterface::kit_zip_city :
						// Auswahl fuer Postleitzahl und Stadt
						$form_fields[$field_name] = array('id' => $field_id, 'type' => $field_name, 'name_zip' => kitContactInterface::kit_zip, 'value_zip' => (isset($_REQUEST[kitContactInterface::kit_zip])) ? $_REQUEST[kitContactInterface::kit_zip] : '', 'name_city' => kitContactInterface::kit_city, 'value_city' => (isset($_REQUEST[kitContactInterface::kit_city])) ? $_REQUEST[kitContactInterface::kit_city] : '', 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'label' => $kitContactInterface->field_array[$field_name], 'hint' => $this->lang->translate('hint_' . $field_name));
						break;
					case kitContactInterface::kit_newsletter :
						...
						$form_fields[$field_name] = array('id' => $field_id, 'type' => $field_name, 'name' => $field_name, 'value' => '', 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'label' => $kitContactInterface->field_array[$field_name], 'hint' => $this->lang->translate('hint_' . $field_name), 'newsletters' => $newsletter_array);
						break;
					case kitContactInterface::kit_country :
						$country_array = array();
						$country_array[] = array('value' => '', 'text' => $this->lang->translate('- select country -'));
						...
						$form_fields[$field_name] = array('id' => $field_id, 'type' => $field_name, 'name' => $field_name, 'value' => 'DE', 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'label' => $kitContactInterface->field_array[$field_name], 'hint' => $this->lang->translate('hint_' . $field_name), 'countries' => $country_array);
						break;
					default :
						// Datentyp nicht definiert - Fehler ausgeben
						$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('The datatype <b>{{ type }}</b> is not supported!', array('type' => $field_name))));
						return false;
				endswitch
				;
			} else {
				...
				switch ($field[dbKITformFields::field_type]) :
					...
					case dbKITformFields::type_delayed :
						...
						$form_fields[$field[dbKITformFields::field_name]] = array('id' => $field[dbKITformFields::field_id], 'type' => $field[dbKITformFields::field_type], 'name' => $field[dbKITformFields::field_name], 'hint' => $field[dbKITformFields::field_hint], 'label' => $field[dbKITformFields::field_title], 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'value' => $field[dbKITformFields::field_value], 'checkbox' => array('text' => (isset($type_add['text'])) ? $type_add['text'] : '', 'name' => $field[dbKITformFields::field_name], 'value' => $field[dbKITformFields::field_value], 'checked' => (isset($_REQUEST[$field[dbKITformFields::field_name]])) ? 1 : 0));
						break;
					...
					case dbKITformFields::type_file :
						...
						$form_fields[$field[dbKITformFields::field_name]] = array('id' => $field[dbKITformFields::field_id], 'type' => $field[dbKITformFields::field_type], 'name' => $field[dbKITformFields::field_name], 'hint' => $field[dbKITformFields::field_hint], 'label' => $field[dbKITformFields::field_title], 'must' => (in_array($field_id, $must_array)) ? 1 : 0, 'settings' => $settings, 'upload_id' => $upload_id, 'file_desc' => $file_desc, 'file_ext' => $file_ext, 'file_size' => $settings['max_file_size']['value'] * 1024 * 1024, 'select_file' => $this->lang->translate('Select File'));
						$is_file_upload = true;
						break;
					...
					default :
						// continue;
						$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('The datatype <b>{{ type }}</b> is not supported!', array('type' => $field[dbKITformFields::field_type]))));
						return false;
				endswitch
				;
			}
		...
		if ($is_feedback_form) {
			...
			$data = array('WB_URL' => WB_URL, 'form' => $form_data, 'fields' => $form_fields);
	...
	protected function checkForm() {
						...
						if ($_FILES[$file[dbKITformFields::field_name]]['error'] == UPLOAD_ERR_OK) {
							...
							if ($checked) {
								...
								if ($_FILES[$file[dbKITformFields::field_name]]['size'] > ($settings['max_file_size']['value'] * 1024 * 1024)) {
									$message .= $this->lang->translate('<p>The file size exceeds the limit of {{ size }} MB.</p>', array('size' => $settings['max_file_size']));
									$checked = false;
								}
							}
							if (!$checked) {
							...
							// add notice to KIT
							$kitContactInterface->addNotice($contact_id, $this->lang->translate('[kitForm] File <a href="{{ link }}">{{ file }}</a> uploaded.', array('link' => WB_URL . '/modules/kit/kdl.php?id=' . $file_id, 'file' => $mf)));
		...
		if ($checked) {
			...
			$check_array = array(kitContactInterface::kit_free_field_1, kitContactInterface::kit_free_field_2, kitContactInterface::kit_free_field_3, kitContactInterface::kit_free_field_4, kitContactInterface::kit_free_field_5, kitContactInterface::kit_free_note_1, kitContactInterface::kit_free_note_2);
			...
			foreach ($fields as $fid) {
				...
				endswitch
				;
				$items[$field[dbKITformFields::field_name]] = array('label' => $field[dbKITformFields::field_title], 'value' => $value, 'type' => $field[dbKITformFields::field_type]);
			}
	...
	protected function delayedTransmission($data_id, $contact_id) {
		...
		$data = array(dbKITformCommands::FIELD_COMMAND => $cmd_delayed, dbKITformCommands::FIELD_PARAMS => http_build_query($form_data), dbKITformCommands::FIELD_TYPE => dbKITformCommands::TYPE_DELAYED_TRANSMISSION, dbKITformCommands::FIELD_STATUS => dbKITformCommands::STATUS_WAITING);
		...
		// client
		$data = array('form' => array('subject' => $form[dbKITform::field_title], 'title' => $form[dbKITform::field_title], 'link' => sprintf('%s?%s#%s', $this->page_link, http_build_query(array(self::request_action => self::action_command, self::request_command => $cmd_delayed)), self::FORM_ANCHOR)), 'contact' => $contact);
		...
		$mail = new kitMail($form[dbKITform::field_provider_id]);
		if (!$mail->mail($client_subject, $client_mail, $provider_data['email'], $provider_data['email'], array($contact[kitContactInterface::kit_email] => $contact[kitContactInterface::kit_email]), ($form[dbKITform::field_email_html] == dbKITform::html_on) ? true : false)) {
	...
	protected function isDirectoryEmpty($directory) {
		...
		if (false === ($handle = @opendir($directory))) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('Can\'t open the directory <b>{{ directory }}</b>!', array('directory' => $directory))));
			return false;
		}
		// read directory
	...
	protected function checkLogin($form_data = array()) {
		...
		if (!$kitLibrary->validateEMail($_REQUEST[kitContactInterface::kit_email])) {
			unset($_REQUEST[kitContactInterface::kit_password]);
			$this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> is not valid, please check your input.</p>', array('email' => $_REQUEST[kitContactInterface::kit_email])));
			return $this->showForm();
		}
		$contact = array();
	...
	protected function showFeedbackForm($form_id, $form_data, $form_fields) {
		...
		foreach ($feedbacks as $feedback) {
			...
			if ($publish && ($fields[$fb_url] == $url)) {
				$feedback_array[] = array('url' => $url, 'subject' => isset($fields[$fb_subject]) ? $fields[$fb_subject] : '', 'text' => isset($fields[$fb_text]) ? $fields[$fb_text] : '', 'homepage' => isset($fields[$fb_homepage]) ? $fields[$fb_homepage] : '', 'nickname' => isset($fields[$fb_nickname]) ? $fields[$fb_nickname] : '', 'date' => array('timestamp' => $feedback[dbKITformData::field_date], 'formatted' => date(cfg_datetime_str, strtotime($feedback[dbKITformData::field_date]))));
			}
		}
		$data = array('feedback' => array('items' => $feedback_array, 'count' => count($feedback_array)), 'form' => $form_data, 'fields' => $form_fields);
	...
	protected function checkFeedbackForm($form_data = array(), $contact_data = array(), $data_id) {
		...
		$data = array(dbKITformCommands::FIELD_COMMAND => $cmd_publish, dbKITformCommands::FIELD_PARAMS => http_build_query(array('form' => $form, 'contact' => $contact_data, 'data_id' => $data_id)), dbKITformCommands::FIELD_TYPE => dbKITformCommands::TYPE_FEEDBACK_PUBLISH, dbKITformCommands::FIELD_STATUS => dbKITformCommands::STATUS_WAITING);
		if (!$dbKITformCommands->sqlInsertRecord($data)) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $dbKITformCommands->getError()));
			return false;
		}
		
		$data = array(dbKITformCommands::FIELD_COMMAND => $cmd_refuse, dbKITformCommands::FIELD_PARAMS => http_build_query(array('form' => $form, 'contact' => $contact_data, 'data_id' => $data_id)), dbKITformCommands::FIELD_TYPE => dbKITformCommands::TYPE_FEEDBACK_REFUSE, dbKITformCommands::FIELD_STATUS => dbKITformCommands::STATUS_WAITING);
		if (!$dbKITformCommands->sqlInsertRecord($data)) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $dbKITformCommands->getError()));
			return false;
		}
		...
		$data = array('feedback' => array('field' => $feedback_array, 'unsubscribe_link' => sprintf('%s?%s#%s', $this->page_link, http_build_query(array(self::request_action => self::action_feedback_unsubscribe, self::request_form_id => $form_id)), self::FORM_ANCHOR)), 'contact' => $contact_data, 'command' => array('publish_feedback' => sprintf('%s?%s#%s', $this->page_link, http_build_query(array(self::request_action => self::action_command, self::request_command => $cmd_publish)), self::FORM_ANCHOR), 'refuse_feedback' => sprintf('%s?%s#%s', $this->page_link, http_build_query(array(self::request_action => self::action_command, self::request_command => $cmd_refuse)), self::FORM_ANCHOR)));
		...
		$mail = new kitMail($form[dbKITform::field_provider_id]);
		if (!$mail->mail($client_subject, $client_mail, $provider_email, $provider_name, array($contact_data[kitContactInterface::kit_email] => $contact_data[kitContactInterface::kit_email]), ($form[dbKITform::field_email_html] == dbKITform::html_on) ? true : false)) {
			$err = $mail->getMailError();
			if (empty($err))
		...
		if ($feedback_array[self::FIELD_FEEDBACK_PUBLISH] == self::PUBLISH_IMMEDIATE) {
			$message .= $this->lang->translate('<p>Thank you for the feedback!</p><p>Your feedback is already published, we have send you a copy to your email address <b>{{ email }}</b>.</p>', array('email' => $contact_data[kitContactInterface::kit_email]));
		} else {
			$message .= $this->lang->translate('<p>Thank your for the feedback!</p><p>We will check and publish your feedback as soon as possible. We have send you a copy of your feedback to your email address <b>{{ email }}</b>.</p>', array('email' => $contact_data[kitContactInterface::kit_email]));
		}
	...
	protected function checkFeedbackUnsubscribe() {
		...
		if (!$kitLibrary->validateEMail($email)) {
			$this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> is not valid, please check your input.</p>', array('email' => $email)));
		...
		if (!$kitContactInterface->isEMailRegistered($email, $contact_id, $status)) {
			...
			$this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> is not registered.</p>', array('email' => $email)));
			return $this->showFeedbackUnsubscribe();
		}
		
		// search for form datas for this user
		$where = array(dbKITformData::field_kit_id => $contact_id, dbKITformData::field_form_id => $form_id);
		$form_data = array();
		if (!$dbKITformData->sqlSelectRecord($where, $form_data)) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $dbKITformData->getError()));
			return false;
		...
		$where = array(dbKITformFields::field_form_id => $form_id, dbKITformFields::field_name => self::FIELD_FEEDBACK_SUBSCRIPTION);
		$fields = array();
		...
		$where = array(dbKITformFields::field_form_id => $form_id, dbKITformFields::field_name => self::FIELD_FEEDBACK_URL);
		$fields = array();
		if (!$dbKITformFields->sqlSelectRecord($where, $fields)) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $dbKITformFields->getError()));
			return false;
		...
		foreach ($form_data as $data) {
			...
			if (isset($values[$fb_subscription][0]) && isset($values[$fb_url])) {
				if (($values[$fb_subscription][0] == self::SUBSCRIPE_YES) && ($values[$fb_url] == $url)) {
					...
					$where = array(dbKITformData::field_id => $data[dbKITformData::field_id]);
					$upd = array(dbKITformData::field_values => http_build_query($values), dbKITformData::field_timestamp => date('Y-m-d H:i:s'));
		...
		if ($unsubscribed) {
			$this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> does no longer receive messages at new feedbacks on this page.</p><p>The settings of other pages are not changed!</p>', array('email' => $email)));
		} else {
			$this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> does not receive any messages from this page, so nothing was changed.</p>', array('email' => $email)));
		}
		return $this->showForm();
	...
	protected function sendNewPassword($form_data = array()) {
		...
		if (!$kitContactInterface->isEMailRegistered($_REQUEST[kitContactInterface::kit_email], $contact_id, $status)) {
			...
			$this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> is not registered.</p>', array('email' => $_REQUEST[kitContactInterface::kit_email])));
			return $this->showForm();
		}
		if ($status != dbKITcontact::status_active) {
			// Der Kontakt ist NICHT AKTIV!
			$this->setMessage($this->lang->translate('<p>The account for the email address <b>{{ email }}</b> is not active, please contact the service!</p>', array('email' => $_REQUEST[kitContactInterface::kit_email])));
			return $this->showForm();
		}
		// CAPTCHA pruefen?
	...
	protected function registerAccount($form_data = array(), $contact_data = array()) {
		...
		if ($kitContactInterface->isEMailRegistered($contact_data[kitContactInterface::kit_email], $contact_id, $status)) {
			...
			if ($status == dbKITcontact::status_active) {
				// Kontakt ist aktiv
				$this->setMessage($this->lang->translate('<p>The email address <b>{{ email }}</b> is already registered, please login with your user data!</p>', array('email' => $contact_data[kitContactInterface::kit_email])));
				return $this->showForm();
			} else {
				// Kontakt ist gesperrt
				$this->setMessage($this->lang->translate('<p>The account for the email address <b>{{ email }}</b> is locked. Please contact the service!</p>', array('email' => $contact_data[kitContactInterface::kit_email])));
				return $this->showForm();
			}
		} elseif ($kitContactInterface->isError()) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $kitContactInterface->getError()));
		...
		$form_data['activation_link'] = sprintf('%s%s%s', $this->page_link, (strpos($this->page_link, '?') === false) ? '?' : '&', http_build_query(array(self::request_action => self::action_activation_key, self::request_key => $register_data[dbKITregister::field_register_key], self::request_provider_id => $form_data[dbKITform::field_provider_id], self::request_activation_type => self::activation_type_account)));
		$form_data['subject'] = $form_data[dbKITform::field_title];
		...
		if (!$mail->mail($client_subject, $client_mail, $provider_email, $provider_name, array($contact_data[kitContactInterface::kit_email] => $contact_data[kitContactInterface::kit_email]), ($form_data[dbKITform::field_email_html] == dbKITform::html_on) ? true : false)) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('Can\'t send the email to <b>{{ email }}</b>!', array('email' => $contact_data[kitContactInterface::kit_email]))));
			return false;
		}
	...
	protected function checkActivationKey() {
		...
		if (!isset($_REQUEST[self::request_key])) {
			$this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $this->lang->translate('Missing the datafield <b>{{ field }}</b>!', array('field' => self::request_key))));
			return false;
		}
